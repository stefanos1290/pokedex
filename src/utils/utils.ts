...
export const fetchPokemonData = async (
  pokemonId: string
): Promise<IPokemonCompleteData> => {
  // 1. Grab pokemon data
  const originalPokemonData = await fetchPokemon("", pokemonId);
  const originalPokemonExtra = await (
    await fetch(`${baseUrl}pokemon-species/` + originalPokemonData.id)
  ).json();
  ...
  if (evolutionChainRes.chain.evolves_to.length !== 0) {
    if (evolutionChainRes?.chain?.evolves_to[0]) {
      evolutionPokemonSpeciesData.push(
        evolutionChainRes?.chain?.evolves_to[0].species
      );
    }
    if (evolutionChainRes?.chain?.evolves_to[0].evolves_to[0]) {
      evolutionPokemonSpeciesData.push(
        evolutionChainRes?.chain?.evolves_to[0].evolves_to[0].species
      );
    }
  }
  ...
  const evolusionPokemonExtraData = await Promise.all(
    evolutionPokemonIds.map(async (item) => {
      return await fetchPokemon("", item);
    })
  );

  const evolutionReturnedData: Array<IPokemonData> =
    evolusionPokemonExtraData.map((p) => {
      return {
        name: p.name,
  ...
  if (
    originalPokemonData.types[1] !== undefined &&
    originalPokemonData.types.length > 0
  ) {
